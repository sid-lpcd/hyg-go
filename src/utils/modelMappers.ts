  ...
  static mapPlan(apiPlan: any): Plan | PlanWithActivities {
    ...
    if (apiPlan.activities != null){
      return {
        ...
        title: this.ensureString(apiPlan.title || apiPlan.name) || '',
        description: this.ensureString(apiPlan.description),
        locationId: this.ensureNumber(apiPlan.locationId || apiPlan.location_id) || 0,
        startDate: this.ensureString(apiPlan.startDate || apiPlan.start_date) || new Date().toISOString(),
        endDate: this.ensureString(apiPlan.endDate || apiPlan.end_date) || new Date().toISOString(),
        people: apiPlan.people || { [PersonType.ADULT]: 1 },
        isPublic: Boolean(apiPlan.isPublic || apiPlan.is_public),
        mainImageUrl: this.ensureString(apiPlan.mainImageUrl || apiPlan.main_image_url),
        userImagesTrip: apiPlan.userImagesTrip || apiPlan.user_images_trip,
        tags: Array.isArray(apiPlan.tags) ? apiPlan.tags : [],
        likes: this.ensureNumber(apiPlan.likes) || 0,
        createdAt: this.ensureString(apiPlan.createdAt || apiPlan.created_at) || new Date().toISOString(),
        updatedAt: this.ensureString(apiPlan.updatedAt || apiPlan.updated_at) || new Date().toISOString(),
