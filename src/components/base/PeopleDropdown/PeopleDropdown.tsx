...
export const PeopleDropdown: React.FC<PeopleDropdownProps> = ({ tripData, setTripData, onClose }) => {
  ...
  const handleChangePeople = (field: keyof People, value: number): void => {
    if (field === PersonType.ADULT)
      setLocalPeople({
        ...localPeople,
        [PersonType.ADULT]: Math.max(1, localPeople[PersonType.ADULT] + value),
      });
    if (field === PersonType.CHILD)
      setLocalPeople({
        ...localPeople,
        [PersonType.CHILD]: Math.max(0, localPeople[PersonType.CHILD] + value),
      });
    if (field === PersonType.INFANT)
      setLocalPeople({
        ...localPeople,
        [PersonType.INFANT]: Math.max(0, localPeople[PersonType.INFANT] + value),
      });
  };
    ...
    <>
      <article className="people-dropdown__container">
        ...
        <PeopleControl
          ...
          count={localPeople?[PersonType.CHILD]}
          onChange={(val) => handleChangePeople(PersonType.CHILD, val)}
        />
        <PeopleControl
          label="Infant (0-2yrs):"
          count={localPeople?[PersonType.INFANT]}
